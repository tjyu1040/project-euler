...
def solve():
    grid = np.array([[int(n) for n in line.split()] for line in GRID.splitlines()])
    max_product = 0
    ...
    for diagonal_index in range(-grid_length + 1, grid_length):
        diagonal = np.diagonal(grid, offset=diagonal_index)
...
def get_max_product_of_4_adj_elements_in_array(array: np.ndarray) -> int:
    if len(array) < 4:
        return 0
    else:
        max_product = 0
